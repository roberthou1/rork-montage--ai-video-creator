...
export async function fetchSmartCollections(
  ...
  console.log('[BackendAPI] Fetching smart collections for', videos.length, 'items');

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 15000);

  try {
    const response = await fetch(buildBackendUrl('/api/collections/smart'), {
      method: 'POST',
      headers: {
        Accept: 'application/json',
        'Content-Type': 'application/json',
    ...
    if (!response.ok) {
      const errorMessage = await readErrorMessage(response);
      console.error('[BackendAPI] Smart collections failed:', response.status, errorMessage);
      return { collections: [] };
    }

    const data = (await response.json()) as FetchSmartCollectionsResponse;
    console.log('[BackendAPI] Smart collections result:', data.collections.length, 'collections');
    return data;
  } catch (error) {
    clearTimeout(timeout);
    console.error('[BackendAPI] Smart collections error:', error);
    return { collections: [] };
  }
}
